...
| 39 | Rename scan files in place between naming schemes (torrent/Yale/project/Rene/etc., driven by `data/scan-naming.tsv`) | ✅ File → Rename to… (submenu lists every TSV column except the current `Config.namingScheme`; confirms first, warns on ≤10 or >213 files found, re-runs Scan on success) | ❌ | `Voynich.renameScans()` → `ScanRenamer` / `RenameTaskWindow` (added 2026-08-18) |
