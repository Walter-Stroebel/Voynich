...
| `ScanRenamer` | Loads the bundled `data/scan-naming.tsv` (tab-separated: `Id` (permanent `CatalogEntry.id`, excluded from `columns`) plus `Sequential`/`Yale`/`VoynichNu` columns so far, one row per manuscript page, cells blank where a scheme has no name for an irregular/non-foliated page — see `SCANS.md`) as a plain 1:1 naming dictionary, no image knowledge involved. `cached()` is the shared load-once/remember-failure static cache every caller (`OverviewPanel`, `CatalogCli`, `CatalogEntryEditor`) uses rather than each hand-rolling its own. `idForFolio(number, side)` resolves a folio reference to its permanent id via the `Yale` column — the shared half of the recto/verso counterpart lookup `OverviewPanel`/`CatalogCli` each finish differently (in-memory grid vs. `Catalog.listAll()` scan). `plan(scanDir, fromColumn, toColumn)` matches files actually present in `scanDir` against `fromColumn` (by basename, extension-agnostic — a file's real extension can differ from whatever a naming column's own value carries) and works out each one's destination name under `toColumn`, always preserving the source file's real extension rather than adopting the target column's. Returns one `Plan` per matched file *before* touching the filesystem, so a caller can detect target-name collisions across the whole batch and refuse those individually (clear `skipReason`) without choking on the rest of a large batch — no such collision exists in the current three-column table, but an earlier four-column version (with a since-dropped `yale_label` column) did, and the guard stays in place for whatever the next added column turns up. A file already correctly named under `toColumn` is a no-op skip, not a collision. `execute(plans, listener)` performs the non-skipped plans via plain `File.renameTo`, notifying a `PlanListener` per file. |
| `RenameTaskWindow` | `TaskWindow` behind File → "Rename to…" — runs one `ScanRenamer.execute` batch, logs each file's outcome, and on any successful rename updates `Config.namingScheme` to the new scheme and (via an `onDone` callback from `Voynich`) re-triggers Scan, since the catalog itself is still keyed by the old filenames until Scan reconciles it — a rename alone only touches the filesystem. |
| `Catalog` | Persistence contract for the image catalog: one `CatalogEntry` (thumbnail inlined as base64) per permanent id. `Catalog.open(Config)` opens the `FileCatalog` backend. |
| `CatalogEntry` | JSON-serializable catalog record, keyed by permanent id (not filename or path) — see "Catalog persistence" below. |
