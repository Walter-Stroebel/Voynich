...
| `WorkingAreaCanvas` | Interactive tracing surface for one entry's `CatalogEntry.workingArea` polygon over its full-resolution image: click to place vertices around the manuscript page's actual boundary, click near the start to close, drag any handle to adjust afterward. The live segment to the cursor while tracing is drawn via `Graphics2D.setXORMode`, not a repaint — the same line drawn twice cancels out, so a mouse-move just erases-and-redraws that one segment directly instead of re-rendering the whole (often very large) image on every pixel of cursor travel. |
| `WorkingAreaEditor` | Wraps `WorkingAreaCanvas` with Clear/Commit/Cancel controls and opens it via `ViewFrame` (`maximizeInitially=true` — every pixel of screen matters for precise tracing), spawned by `CatalogEntryEditor`'s "Working Area" button. See its class doc for why `workingArea` is human-traced rather than auto-detected: the "this page" vs. "the pages stacked underneath it" boundary isn't a colour/lightness distinction (same material), and no fold — however severe — is ever a true boundary, since manuscript content routinely runs right through them. |
| `BitSet2D` | A bit-per-pixel 2D mask (`BitSet` under the hood — real word-level range operations, not one call per pixel) plus utilities: flood fill (`oilSpill`), grow/shrink, invert, image conversion. `createFromPolygon` rasterizes a `List<Point>` (e.g. a decoded `CatalogEntry.workingArea`) via a real scanline fill straight into the bits — deliberately not through `java.awt.Shape#contains`, which is the trap `createFromShape`/`copy`/`setOrClear` (kept, `@Deprecated`) fall into: recomputing the winding number from every edge on every pixel query is fine once, ruinous over millions. First consumer: `CatalogEntryEditor`'s "Show Mask" toggle. |
...
| `CatalogCli` | Command-line access to the catalog (`list`, with an optional case-insensitive/invertible text filter over an entry's whole JSON; `get`/`tag`/`save`; `checkpoint`/`restore`), through the same `Catalog.open(Config)` the GUI uses — works against either backend. Run via `java -cp target/Voynich-1.0-jar-with-dependencies.jar nl.infcomtec.voynich.CatalogCli <command>`, bypassing the fat jar's GUI `Main-Class`. `extract` pulls real decoded pixels: `--pixel x,y`/`--region x,y,w,h` (repeatable) go through `ColorImage`/`ColorBase` for rgb/lab/hex output, same colour math the GUI views use; `--working-area` skips that (no Lab decode needed for a raw crop) and instead writes a PNG cropped to `CatalogEntry.workingArea`'s bounding box, black outside the polygon (via `BitSet2D.createFromPolygon`). |
...
`CatalogEntry.workingArea` (a `List<CatalogEntry.Vertex>`, empty until traced)
is the polygon boundary of the scan's actual manuscript page, excluding
photography backdrop, frayed vellum edges, and the other pages visible in
the stack beneath it — set via `WorkingAreaEditor`, never auto-detected (see
its class doc). Same "no migration needed" story as `torrentJpg`/`tags`.
