...
# Build fat jar
mvn package
# Run (fat jar) — the * picks up whatever version pom.xml currently declares
java -jar target/Voynich-*-jar-with-dependencies.jar [optional-config-file]
# Run via Maven
mvn exec:java
...
java -jar target/Voynich-*-jar-with-dependencies.jar --smokeTest
```
...
## Configuration
On first launch the app creates its data folder if missing (via `MitsaPaths.appDataDir("voynich")` — `~/.config/mitsa/data/voynich/` on Linux/Mac, `%APPDATA%\mitsa\data\voynich\` on Windows; fails fast if that path exists but isn't a directory), then writes a template config to stderr and exits with code 2 if `config.json` there is missing or `scanPath` is unset. Create the file manually:
```json
{
...
### Catalog persistence
`Catalog.open(config)` opens `FileCatalog` rooted at `<Voynich.baseDir>/catalog`, one
...
under `<Voynich.baseDir>/catalog-checkpoints`, cheaper on disk than a raw directory
...
`Voynich.baseDir`, resolved via `MitsaPaths.appDataDir("voynich")` —
nested under [MITSA](https://github.com/Walter-Stroebel/mitsa)'s own
per-OS config root (`~/.config/mitsa/data/voynich/` on Linux/Mac,
`%APPDATA%\mitsa\data\voynich\` on Windows), created on class load if
missing. `MitsaPaths` (`src/main/java/nl/infcomtec/mitsa/MitsaPaths.java`)
is a hand-copied file, not a Maven dependency — MITSA isn't published to
a repository, same file-copy convention as jacksonwrap/advswing; keep it
in sync by hand if MITSA's own copy changes. `Voynich.configFile` and
`Catalog.open` both derive from `baseDir`, and `CatalogCli` reuses
`Voynich.configFile` rather than resolving its own copy.
