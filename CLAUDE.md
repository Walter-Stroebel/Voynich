...
| `ViewFrame` | Opens a named, independent tool window (a `JDialog` owned by the long-lived main app frame, for correct window-manager stacking — not by whatever transient dialog spawned it) around one visualization `JComponent`, remembering that name's on-screen bounds in `Config.viewBounds` across restarts. Named per view *type* (e.g. "Color Frequency"), not per image, so reopening the same visualization for a different entry lands wherever it was last left. |
